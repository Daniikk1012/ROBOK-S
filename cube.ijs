...
PERMUTATIONS =: (#TURNS_roboksparser_)$a:
append =. {{PERMUTATIONS =: ,&y&.>&.((TURNS_roboksparser_ i.x)&{)PERMUTATIONS}}
SIDES_roboksparser_ append items(0 6 8 2;1 3 7 5)&(+each)every 9*i.6
...
'r'append(,|.each)&>/(TURNS_roboksparser_ i.'RM'){PERMUTATIONS
'u'append(,|.each)&>/(TURNS_roboksparser_ i.'UE'){PERMUTATIONS
'f'append ,       &>/(TURNS_roboksparser_ i.'FS'){PERMUTATIONS
'l'append ,       &>/(TURNS_roboksparser_ i.'LM'){PERMUTATIONS
'd'append ,       &>/(TURNS_roboksparser_ i.'DE'){PERMUTATIONS
'b'append(,|.each)&>/(TURNS_roboksparser_ i.'BS'){PERMUTATIONS
'x'append(,|.each)&>/(TURNS_roboksparser_ i.'rL'){PERMUTATIONS
'y'append(,|.each)&>/(TURNS_roboksparser_ i.'uD'){PERMUTATIONS
'z'append(,|.each)&>/(TURNS_roboksparser_ i.'fB'){PERMUTATIONS
...
rotate =: {{
  ...
  case. AST_CONDITIONS_roboksparser_ do.
    r =. 0$~0,$y
    for_term. >{:x do.
      'cond body' =. term
      if. #cond do.
  ...
  case. AST_CONDITIONS_roboksparser_ do. throw.
  end.
}}

NB. Prints the cube state y in a (almost) pretty format
display =: {{
  s =. ,((3$' '),{&SIDES_roboksparser_,LF"_)rows(SIDES_roboksparser_ i.'U'){y
  s =. s,,({&SIDES_roboksparser_,LF"_)rows,./(SIDES_roboksparser_ i.'LFRB'){y
  s =. s,,((3$' '),{&SIDES_roboksparser_,LF"_)rows(SIDES_roboksparser_ i.'D'){y
  echo}:s
}}
...
scramble =: {{
  r =. ,?6
  for. i.17+?5 do.
    if. 3=|-/_2{.!.(#SIDES_roboksparser_)r do.
      r =. r,(+(1 2-~sort(,6|+&3){:r)&I.)?2-~#SIDES_roboksparser_
    else.
      r =. r,>:^:(({:r)&<:)?<:#SIDES_roboksparser_
    end.
  end.
  }.;(r{SIDES_roboksparser_)' '&,@,each('';'''';'2'){~?3$~#r
}}
